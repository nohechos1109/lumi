...
interface Props {
  ...
  creditDisponible: string  // crédito disponible del cliente
  onClose: () => void
  onApplied: () => void
}
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/cobranza/credit/apply', {
        ...
        body: JSON.stringify({ note_id: noteId, amount: n }),
      })
      if (res.ok) {
        onApplied()
        onClose()
        ...
        setError(d.error ?? 'Error al aplicar crédito')
      }
    } finally {
      setLoading(false)
    }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-base font-bold" style={{ color: 'var(--c-ink)' }}>Aplicar Crédito</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <p className="text-xs rounded-lg px-3 py-2" style={{ background: '#EFF6FF', color: '#1D4ED8' }}>
            El crédito se tomará de los pagos más antiguos del cliente.
