...
export function CustomerFormModal({ customer, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (type === 'person') {
      fetch('/api/customers')
        .then(r => r.json())
        .then((data: ContactSaved[]) => setCompanies(data.filter(c => c.type === 'company').map(c => ({ id: c.id, name: c.name }))))
        .catch(() => {})
    }
  }, [type])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Sync company links for persons
      if (type === 'person') {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Type toggle */}
        {!isEdit && (
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid var(--c-rim)' }}>
            {(['company', 'person'] as const).map(t => (
              <button
                ...
                style={{
                  background: type === t ? 'var(--c-navy)' : 'var(--c-panel)',
                  color: type === t ? '#fff' : 'var(--c-dim)',
                }}
              >
            ...
            <>
              <Field label="Nombre / Razón social" required>
                <input
                  ...
                  placeholder="Empresa S.A. de C.V." required autoFocus
          ...
          {type === 'person' && (
            <div className="flex flex-col gap-2">
              ...
              {linkedCompanies.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  {linkedCompanies.map((c, i) => (
                    <div
                      key={c.id}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm"
                      style={{ background: 'var(--c-panel)', border: c.is_primary ? '1px solid var(--c-navy-bd)' : '1px solid var(--c-rim)' }}
                    >
                      ...
                      <input
                        ...
                        style={{ background: 'var(--c-card)', border: '1px solid var(--c-rim)', color: 'var(--c-dim)' }}
                  ...
                  <div className="relative flex-1">
                    ...
                    {showCompanyDropdown && filteredCompanies.length > 0 && (
                      <div
                        ...
                        style={{ background: 'var(--c-card)', border: '1px solid var(--c-rim)', maxHeight: '180px', overflowY: 'auto' }}
                      >
                    ...
                    {showCompanyDropdown && companySearch && filteredCompanies.length === 0 && (
                      <div
                        ref={dropdownRef}
                        className="absolute z-20 left-0 right-0 top-full mt-1 rounded-lg px-3 py-2 text-sm shadow-lg"
                        style={{ background: 'var(--c-card)', border: '1px solid var(--c-rim)', color: 'var(--c-ghost)' }}
                      >
                        Sin resultados
                      </div>
                    )}
